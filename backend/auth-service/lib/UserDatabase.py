...
class UserDatabase(DataBase):
    ...
    def create(self,user_data:UserData) -> None:
        ...
        try:
            new_user = self.User(user_name = user_data['user_name'],
    ...
    def query(self,user_name:str) -> User|None:
        session = self.Session()
        query_user = session.query(self.User).filter(self.User.user_name == user_name).first()
        session.close()
        return query_user
    ...
    def update(self,user_name:str,hashed_paaword:str,mail:str) -> None:
        ...
        session = self.Session()
        try:
            update_user = session.query(self.User).filter(self.User.user_name == user_name).first()
            if update_user:
